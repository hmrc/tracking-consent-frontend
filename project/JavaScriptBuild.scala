...
object JavaScriptBuild {
  ...
  val npmNpxInstall = TaskKey[Int]("npm-npx-install")
  ...
  val javaScriptSettings: Seq[Setting[_]] = Seq(
    javaScriptDirectory := (baseDirectory in Compile) {
      _ / "app" / "uk" / "gov" / "hmrc" / "trackingconsentfrontend" / "assets"
    }.value,
    ...
    npmNpxInstall := Npm
      .npmProcess("npm install axe failed")(javaScriptDirectory.value, "install", "npx", "-g"),
    npmAxeInstall := Npm
    ...
    (test in Test) := (test in Test).dependsOn(npmNpxInstall).value
  )
}
